...
/*void cardScan2() {
  ...
  for (byte i = 0; i < mfrc522_2.uid.size; i++) 
  {
     ...
     Serial.println(mfrc522_2.uid.uidByte[i], HEX);
...
void setColor() {
  ...
  hue2.setHueSat(ID2, color/scan, sat/scan);
}
...
void setSat() {
  ...
  if (currentState1 == 1 && currentState2 == 1 && valueSat <= 250) {
    ...
    hue2.setHueSat(ID2, color/scan, sat);
    Serial.println("Sat+");
    delay(100);
    setSat();
    }
